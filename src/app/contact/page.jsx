...
export async function generateMetadata() {
  ...
  const page   = getMdxSinglePage('contact');
  const fm     = page?.frontmatter ?? {};
  return {
    title:       fm.meta_title       ?? `Contact ${config.business_name} | ${config.city}, ${config.state}`,
    description: fm.meta_description ?? `Contact ${config.business_name} for a free estimate. Serving ${config.city}, ${config.state} and surrounding areas. Call ${config.phone ?? 'today'}.`,
  };
}
...
export default async function ContactPage() {
  ...
  // Contact MDX content — generated by content engine, lives at content/contact.mdx
  const contactPage = getMdxSinglePage('contact');
  let mdxContent = null;
  if (contactPage?.content) {
    const result = await compileMDX({ source: contactPage.content, options: { parseFrontmatter: false } });
  ...
  return (
    <>
      ...
      {/* Contact MDX intro — service area copy, response time, what to expect */}
      {mdxContent && (
        <section className="section-pad-sm bg-surface">
          <div className="container">
            <article className="mdx-content" style={{ fontSize: '1.0rem', lineHeight: 1.75, color: 'var(--color-text-body)', maxWidth: 720, margin: '0 auto' }}>
              {mdxContent}
            </article>
          </div>
        </section>
